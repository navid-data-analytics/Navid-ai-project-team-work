...
func TestWrapServerStream(t *testing.T) {
	ctx := context.WithValue(context.TODO(), key("something"), 1)
	fake := &fakeServerStream{ctx: ctx}
	wrapped := WrapServerStream(fake)
	assert.NotNil(t, wrapped.Context().Value(key("something")), "values from fake must propagate to wrapper")
	wrapped.WrappedContext = context.WithValue(wrapped.Context(), key("other"), 2)
	assert.NotNil(t, wrapped.Context().Value(key("other")), "values from wrapper must be set")
}
